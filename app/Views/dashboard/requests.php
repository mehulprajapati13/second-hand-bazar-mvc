...
<div style="display:flex;gap:0;border-bottom:2px solid #e2e8f0;margin-bottom:20px;">
    <a href="/requests?tab=received"
        ...
        Received
        <?php
        $pendingCount = count(array_filter($received, fn($r) => $r['status'] === 'pending'));
        if ($pendingCount > 0):
        ?>
        ...
        <div style="display:flex;flex-direction:column;gap:12px;">
            <?php foreach ($received as $req):
                $statusBadge = match ($req['status']) {
                    ...
                    default     => ['bg' => '#f1f5f9', 'color' => '#475569', 'text' => ucfirst($req['status'])],
                };
            ?>
                <div class="card" style="padding:16px;">
                    <div style="display:flex;align-items:flex-start;gap:14px;">
                        ...
                        <!-- Item image -->
                        <div style="width:60px;height:52px;border-radius:8px;overflow:hidden;background:#f1f5f9;flex-shrink:0;">
                        ...
                        <div style="flex:1;min-width:0;">
                            <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px;">
                                <div>
                                    <div style="font-weight:600;font-size:0.9rem;"><?= htmlspecialchars($req['item_title']) ?></div>
                                    <div style="font-size:0.8rem;color:#64748b;margin-top:2px;">
                                        From: <strong><?= htmlspecialchars($req['requester_name']) ?></strong>
                                        · <?= date('d M Y', strtotime($req['created_at'])) ?>
                                        · <span style="text-transform:capitalize;"><?= $req['item_mode'] ?></span>
                                    </div>
                                </div>
                                <span style="background:<?= $statusBadge['bg'] ?>;color:<?= $statusBadge['color'] ?>;
                                ...
                                <div style="display:flex;gap:8px;margin-top:10px;">
                                    <form action="/requests/approve/<?= $req['id'] ?>" method="POST">
                                        <button type="submit" class="btn btn-sm"
                                            style="background:#16a34a;color:#fff;border:none;padding:6px 16px;border-radius:6px;font-weight:600;">
                                            ✓ Approve
                                        </button>
                                    </form>
                                    <form action="/requests/reject/<?= $req['id'] ?>" method="POST">
                                        <button type="submit" class="btn btn-sm"
                                            style="background:#fee2e2;color:#991b1b;border:1px solid #fca5a5;padding:6px 16px;border-radius:6px;font-weight:600;">
                                            ✗ Reject
                                        </button>
                                    </form>
                                </div>
                                ...
                                <div style="margin-top:10px;">
                                    <?php if ($req['item_mode'] === 'sell'): ?>
                                        <form action="/requests/sold/<?= $req['id'] ?>" method="POST"
                                            ...
                                            <button type="submit" class="btn btn-sm"
                                                style="background:#1d4ed8;color:#fff;border:none;padding:6px 16px;border-radius:6px;font-weight:600;">
                                                Mark as Sold
                                            </button>
                                        </form>
                                    <?php else: ?>
                                        <form action="/requests/returned/<?= $req['id'] ?>" method="POST"
                                            ...
                                            <button type="submit" class="btn btn-sm"
                                                style="background:#7c3aed;color:#fff;border:none;padding:6px 16px;border-radius:6px;font-weight:600;">
                                                Mark as Returned
                                            </button>
                                        </form>
                                    <?php endif; ?>
                    ...
                    <div style="display:flex;align-items:flex-start;gap:14px;">
                        ...
                        <div style="flex:1;min-width:0;">
                            ...
                            <?php if ($req['status'] === 'approved'): ?>
                                <div style="margin-top:8px;padding:8px 12px;background:#d1fae5;border-radius:8px;font-size:0.8rem;color:#065f46;font-weight:600;">
                                    ✓ Approved! Contact the seller to arrange handover.
                                </div>
                            <?php endif; ?>
                            ...
                            <?php if ($req['status'] === 'pending'): ?>
                                <div style="margin-top:10px;">
                                    <form action="/requests/cancel/<?= $req['id'] ?>" method="POST"
                                        ...
                                        <button type="submit" class="btn btn-sm"
                                            style="background:#f1f5f9;color:#475569;border:1px solid #e2e8f0;padding:6px 16px;border-radius:6px;font-weight:600;">
                                            Cancel Request
                                        </button>
                                    </form>
                                </div>
